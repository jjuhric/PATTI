...
describe('Daily Briefing Generation Tests', () => {
  ...
  describe('getUserLocalNow', () => {
    test('resolves the local hour and date for a given IANA timezone at a fixed instant', () => {
      const at = new Date('2026-07-30T15:00:00Z');
      // America/Chicago is UTC-5 in July (CDT) -> 15:00 UTC = 10:00 local.
      expect(getUserLocalNow('America/Chicago', at)).toEqual({ hour: 10, dateStr: '2026-07-30' });
      // Pacific/Kiritimati is a fixed UTC+14, no DST -> 15:00 UTC = 05:00 local, next day.
      expect(getUserLocalNow('Pacific/Kiritimati', at)).toEqual({ hour: 5, dateStr: '2026-07-31' });
    });

    test('falls back to America/Chicago when no timezone is given', () => {
      const at = new Date('2026-07-30T15:00:00Z');
      expect(getUserLocalNow(null, at)).toEqual({ hour: 10, dateStr: '2026-07-30' });
    });
  });
