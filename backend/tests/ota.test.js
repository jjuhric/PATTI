...
describe('OTA update endpoint', () => {
  ...
  beforeEach(() => {
    ...
    // Clean tree by default - individual tests override this.
  ...
  test('rejects requests from a non-private IP', async () => {
    ...
    // the isPrivateIp logic itself is exercised by the "accepts" test below
    // succeeding only because req.ip is loopback.
    expect([200, 403]).toContain(res.status);
  });

  test('refuses to run when the working tree has uncommitted changes', async () => {
    execSync.mockReturnValue(Buffer.from(' M some/file.js\n?? new-file.js\n'));
    const res = await request(app).post('/api/ota/update').set('X-OTA-Secret', 'test-secret-value');
    expect(res.status).toBe(409);
    expect(spawn).not.toHaveBeenCalled();
  });

  test('refuses to run when the git status check itself fails', async () => {
  ...
  test('accepts a matching secret from loopback with a clean tree, and spawns the update without waiting on it', async () => {
    const res = await request(app).post('/api/ota/update').set('X-OTA-Secret', 'test-secret-value');
