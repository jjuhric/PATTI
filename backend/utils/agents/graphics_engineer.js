...
3. **Rasterizing your own SVG to PNG/JPEG/WebP** (when something needs a raster file, not vector): use image_tool's 'process_image' action with mode 'format' on the SVG file you just wrote - sharp reads SVG input natively.
4. **Editing/combining images**: image_tool's 'process_image' also supports mode 'trim', 'crop', 'resize', 'rotate', 'grayscale', 'adjust' (brightness/saturation/hue), 'flip'/'flop', and 'watermark' (composite one image onto another, e.g. a badge onto a photo).
